...
(define (list-traverse slow fast p1 p2 steps)
  (let ([p1 (slow p1)]
        [p2 (fast p2)])
    (cond
      [(= p1 p2) (cons p1 steps)]
      [else (list-traverse slow fast p1 p2 (+ 1 steps))])))

; Extracts result from traverse
(define (list-traverse/result slow fast p1 p2)
  (car (list-traverse slow fast p1 p2 0)))

; Extracts the count of iteration from traverse 
(define (list-traverse/count slow fast p1 p2)
  (cdr (list-traverse slow fast p1 p2 0)))
